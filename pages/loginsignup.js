...
const registerUser = async (token, router) => {
  ...
  await axios.post('https://muslimtrivia.com/api/v1/auth/register/google', { token })
    .then(response => {
      const { data } = response;
      const { token } = data;
...
const loginUser = async (token, router) => {
  ...
  await axios.post('https://muslimtrivia.com/api/v1/auth/login/google', { token })
    .then(response => {
      const { data } = response;
      const { token } = data;
...
const LoginSignup = () => {
  ...
  const login = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('https://muslimtrivia/api/v1/auth/authenticate', {
        email: signInEmail,
  ...
  const signUp = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('https://54.86.135.3/api/v1/auth/register', {
        userName,
        email,
        password,
